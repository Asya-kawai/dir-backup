...
rotatecmd="find $dstdir -name ${fname}\*01.tar.gz -exec touch '{}' \;"
echo ${rotatecmd}
if [ $debugmode -eq 0 ]; then
    find $dstdir -name ${fname}\*01.tar.gz -exec touch '{}' \;
    check_status $?
fi

# Delete tar.gz-files which not access 5days.
rotatecmd="find $dstdir -name ${fname}\* -atime +5 -exec rm '{}' \;"
echo ${rotatecmd}
if [ $debugmode -eq 0 ]; then
    find $dstdir -name ${fname}\* -atime +5 -exec rm '{}' \;
    check_status $?
fi
